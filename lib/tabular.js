...
Tabular.prototype = {
	...
	findAll: function(limit, output, callback){
		...
		db.collection('visits', function(err, collection){
			...
			collection.find({}, { limit:limit, sort:[['date','desc']] }, function(err, cursor){
				if(err) {
					sys.log(JSON.stringify(err, null, 2));
				}
	...
	renderCsv: function(cursor){
		...
		res.write("Date,IP,Agent,Referrer,Location,Time,JSON\n");
        ...
        sys.log(JSON.stringify(err, null, 2));
				...
				if(item){
					res.write(item.date + "," + item.ip + "," + item.ua + "," + item.referrer + "," + item.location + "," + item.time + "\n");
				} else {
					res.end();
				}
			}			
